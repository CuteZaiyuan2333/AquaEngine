...
public:
  ...
  const std::vector<std::shared_ptr<ShaderModule>> &GetShaders() const {
    return m_shaders;
  }
...
public:
  ...
  CreateShaderFromSource(const std::string &name, const std::string &source,
                         ShaderType type);
...
private:
  ...
  bool CompileGlslToSpirv(const std::string &source, ShaderType type,
