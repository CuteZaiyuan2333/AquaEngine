...
VulkanRenderer::VulkanRenderer()
    : m_window(nullptr), m_instance(nullptr), m_debugMessenger(nullptr),
      ...
      m_renderPass(nullptr), m_pipelineLayout(nullptr), m_graphicsPipeline(nullptr),
      m_swapChainImageFormat(0), m_currentFrame(0), m_currentImageIndex(0) {
...
bool VulkanRenderer::Initialize() {
    ...
    std::cout << "  Size: " << m_config.width << "x" << m_config.height << std::endl;
    std::cout << "  Title: " << m_config.title << std::endl;
    std::cout << "  Validation: " << (m_config.enableValidation ? "enabled" : "disabled") << std::endl;
    std::cout << "  VSync: " << (m_config.enableVSync ? "enabled" : "disabled") << std::endl;
...
bool VulkanRenderer::CreateSurface() {
    ...
    m_surface = surface;
    std::cout << "Vulkan surface created successfully, surface pointer: " << m_surface << std::endl;
    return true;
}
...
bool VulkanRenderer::CreateLogicalDevice() {
    ...
    VkPhysicalDevice physicalDevice = static_cast<VkPhysicalDevice>(m_physicalDevice);
...
bool VulkanRenderer::CreateImageViews() {
    ...
    std::cout << "Image views created successfully (" << m_swapChainImageViews.size() << " views)" << std::endl;
    return true;
}
...
bool VulkanRenderer::CreateGraphicsPipeline() {
    ...
    // Load shaders
    auto vertShaderCode = ReadFile("Shaders/cube_vert.spv");
    auto fragShaderCode = ReadFile("Shaders/cube_frag.spv");
    ...
    pushConstantRange.size = sizeof(float); // Just one float for time
...
bool VulkanRenderer::CreateFramebuffers() {
    ...
    std::cout << "Swap chain image views count: " << m_swapChainImageViews.size() << std::endl;
    std::cout << "Swap chain extent: " << m_swapChainExtent.width << "x" << m_swapChainExtent.height << std::endl;
    
    if (m_swapChainImageViews.empty()) {
        std::cerr << "ERROR: No swap chain image views available for framebuffer creation!" << std::endl;
        return false;
    }
    
    if (m_swapChainExtent.width == 0 || m_swapChainExtent.height == 0) {
        std::cerr << "ERROR: Invalid swap chain extent for framebuffer creation!" << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < m_swapChainImageViews.size(); i++) {
        ...
        if (vkCreateFramebuffer(static_cast<VkDevice>(m_device), &framebufferInfo, nullptr, &framebuffer) != VK_SUCCESS) {
...
bool VulkanRenderer::CreateCommandPool() {
    ...
    uint32_t queueFamilyCount = 0;
    vkGetPhysicalDeviceQueueFamilyProperties(static_cast<VkPhysicalDevice>(m_physicalDevice), &queueFamilyCount, nullptr);
    
    std::vector<VkQueueFamilyProperties> queueFamilies(queueFamilyCount);
    vkGetPhysicalDeviceQueueFamilyProperties(static_cast<VkPhysicalDevice>(m_physicalDevice), &queueFamilyCount, queueFamilies.data());
    ...
    if (vkCreateCommandPool(static_cast<VkDevice>(m_device), &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
...
bool VulkanRenderer::CreateCommandBuffers() {
    ...
    if (vkAllocateCommandBuffers(static_cast<VkDevice>(m_device), &allocInfo, commandBuffers.data()) != VK_SUCCESS) {
...
bool VulkanRenderer::IsDeviceSuitable(void* device) {
    ...
    std::cout << "Formats count: " << swapChainSupport.formats.size() << std::endl;
    std::cout << "Present modes count: " << swapChainSupport.presentModes.size() << std::endl;
...
SwapChainSupportDetails VulkanRenderer::QuerySwapChainSupport(void* device) {
    ...
    if (!m_surface) {
        std::cerr << "Surface is null, cannot query swap chain support" << std::endl;
        return details;
    }
    ...
    std::cout << "Swap chain support query completed: " 
              << formatCount << " formats, " 
...
QueueFamilyIndices VulkanRenderer::FindQueueFamilies(void* device) {
    ...
    std::cout << "FindQueueFamilies: surface pointer = " << m_surface << std::endl;
    ...
    for (const auto& queueFamily : queueFamilies) {
        ...
        if (m_surface) {
            vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, i, static_cast<VkSurfaceKHR>(m_surface), &presentSupport);
            if (presentSupport) {
    ...
    std::cout << "Graphics family: " << (indices.graphicsFamily != UINT32_MAX ? std::to_string(indices.graphicsFamily) : "not found") << std::endl;
    std::cout << "Present family: " << (indices.presentFamily != UINT32_MAX ? std::to_string(indices.presentFamily) : "not found") << std::endl;
...
bool VulkanRenderer::CreateSyncObjects() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        if (vkCreateSemaphore(static_cast<VkDevice>(m_device), &semaphoreInfo, nullptr, &imageAvailableSemaphore) != VK_SUCCESS ||
            vkCreateSemaphore(static_cast<VkDevice>(m_device), &semaphoreInfo, nullptr, &renderFinishedSemaphore) != VK_SUCCESS ||
            vkCreateFence(static_cast<VkDevice>(m_device), &fenceInfo, nullptr, &inFlightFence) != VK_SUCCESS) {
            std::cerr << "Failed to create synchronization objects for frame " << i << std::endl;
            return false;
        }
        
        m_imageAvailableSemaphores[i] = static_cast<void*>(imageAvailableSemaphore);
        m_renderFinishedSemaphores[i] = static_cast<void*>(renderFinishedSemaphore);
...
void VulkanRenderer::Shutdown() {
    ...
    if (m_device != nullptr) {
        ...
        for (size_t i = 0; i < m_imageAvailableSemaphores.size(); i++) {
            if (m_imageAvailableSemaphores[i] != nullptr) {
                vkDestroySemaphore(device, static_cast<VkSemaphore>(m_imageAvailableSemaphores[i]), nullptr);
            }
        }
        
        for (size_t i = 0; i < m_renderFinishedSemaphores.size(); i++) {
            if (m_renderFinishedSemaphores[i] != nullptr) {
                vkDestroySemaphore(device, static_cast<VkSemaphore>(m_renderFinishedSemaphores[i]), nullptr);
            }
        }
        
        for (size_t i = 0; i < m_inFlightFences.size(); i++) {
            if (m_inFlightFences[i] != nullptr) {
                vkDestroyFence(device, static_cast<VkFence>(m_inFlightFences[i]), nullptr);
            }
        }
    }
    ...
    if (m_commandPool != nullptr && m_device != nullptr) {
        vkDestroyCommandPool(static_cast<VkDevice>(m_device), static_cast<VkCommandPool>(m_commandPool), nullptr);
        m_commandPool = nullptr;
    }
    ...
    if (m_device != nullptr) {
        ...
        for (size_t i = 0; i < m_swapChainFramebuffers.size(); i++) {
            if (m_swapChainFramebuffers[i] != nullptr) {
                vkDestroyFramebuffer(device, static_cast<VkFramebuffer>(m_swapChainFramebuffers[i]), nullptr);
            }
        }
    }
    ...
    if (m_graphicsPipeline != nullptr && m_device != nullptr) {
        vkDestroyPipeline(static_cast<VkDevice>(m_device), static_cast<VkPipeline>(m_graphicsPipeline), nullptr);
    ...
    if (m_device != nullptr) {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); i++) {
            if (m_swapChainImageViews[i] != nullptr) {
                vkDestroyImageView(device, static_cast<VkImageView>(m_swapChainImageViews[i]), nullptr);
            }
        }
    }
...
bool VulkanRenderer::BeginFrame() {
    ...
    VkDevice device = static_cast<VkDevice>(m_device);
    VkFence inFlightFence = static_cast<VkFence>(m_inFlightFences[m_currentFrame]);
    ...
    VkSemaphore imageAvailableSemaphore = static_cast<VkSemaphore>(m_imageAvailableSemaphores[m_currentFrame]);
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        std::cerr << "BeginFrame: Failed to acquire swap chain image, result = " << result << std::endl;
        return false;
    }
    ...
    std::cout << "BeginFrame: Resetting command buffer for frame " << m_currentFrame << std::endl;
    VkCommandBuffer commandBuffer = static_cast<VkCommandBuffer>(m_commandBuffers[m_currentFrame]);
    ...
    std::cout << "BeginFrame: Command buffer pointer = " << commandBuffer << std::endl;
    ...
    std::cout << "BeginFrame: vkBeginCommandBuffer result = " << beginResult << std::endl;
    
    if (beginResult != VK_SUCCESS) {
        std::cerr << "BeginFrame: Failed to begin recording command buffer, error code: " << beginResult << std::endl;
        return false;
    }
    
    std::cout << "BeginFrame: Command buffer recording started successfully" << std::endl;
    
    // Begin render pass
    std::cout << "BeginFrame: Setting up render pass info" << std::endl;
    std::cout << "BeginFrame: Image index = " << imageIndex << std::endl;
    std::cout << "BeginFrame: Framebuffers size = " << m_swapChainFramebuffers.size() << std::endl;
    ...
    renderPassInfo.renderArea.extent = {m_swapChainExtent.width, m_swapChainExtent.height};
    ...
    std::cout << "BeginFrame: Render pass = " << renderPassInfo.renderPass << std::endl;
    std::cout << "BeginFrame: Framebuffer = " << renderPassInfo.framebuffer << std::endl;
    std::cout << "BeginFrame: Render area extent = " << renderPassInfo.renderArea.extent.width << "x" << renderPassInfo.renderArea.extent.height << std::endl;
...
void VulkanRenderer::EndFrame() {
    std::cout << "EndFrame: Starting end frame process for frame " << m_currentFrame << std::endl;
    VkDevice device = static_cast<VkDevice>(m_device);
    VkCommandBuffer commandBuffer = static_cast<VkCommandBuffer>(m_commandBuffers[m_currentFrame]);
    ...
    VkSemaphore waitSemaphores[] = {static_cast<VkSemaphore>(m_imageAvailableSemaphores[m_currentFrame])};
    ...
    VkSemaphore signalSemaphores[] = {static_cast<VkSemaphore>(m_renderFinishedSemaphores[m_currentFrame])};
    submitInfo.signalSemaphoreCount = 1;
    submitInfo.pSignalSemaphores = signalSemaphores;
    
    std::cout << "EndFrame: Submitting command buffer to graphics queue" << std::endl;
    VkFence inFlightFence = static_cast<VkFence>(m_inFlightFences[m_currentFrame]);
    if (vkQueueSubmit(static_cast<VkQueue>(m_graphicsQueue), 1, &submitInfo, inFlightFence) != VK_SUCCESS) {
    ...
    std::cout << "EndFrame: Presenting image index " << m_currentImageIndex << std::endl;
    VkResult result = vkQueuePresentKHR(static_cast<VkQueue>(m_presentQueue), &presentInfo);
    
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        std::cout << "EndFrame: Swap chain out of date or suboptimal" << std::endl;
        // Swap chain needs to be recreated
    } else if (result != VK_SUCCESS) {
        std::cerr << "EndFrame: Failed to present swap chain image, result = " << result << std::endl;
    } else {
        std::cout << "EndFrame: Image presented successfully" << std::endl;
    }
    
    m_currentFrame = (m_currentFrame + 1) % MAX_FRAMES_IN_FLIGHT;
    std::cout << "EndFrame: Frame completed, next frame will be " << m_currentFrame << std::endl;
}
...
void VulkanRenderer::RenderMesh(const Mesh& mesh, const Texture* texture) {
    std::cout << "RenderMesh: Starting mesh rendering for frame " << m_currentFrame << std::endl;
    VkCommandBuffer commandBuffer = static_cast<VkCommandBuffer>(m_commandBuffers[m_currentFrame]);
    
    // Bind graphics pipeline
    std::cout << "RenderMesh: Binding graphics pipeline" << std::endl;
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, static_cast<VkPipeline>(m_graphicsPipeline));
    ...
    // Push time constant to shader
    std::cout << "RenderMesh: Pushing time constant: " << m_animationTime << std::endl;
    vkCmdPushConstants(commandBuffer, static_cast<VkPipelineLayout>(m_pipelineLayout), 
                       VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float), &m_animationTime);
    
    // For now, draw a hardcoded cube without vertex buffers (using vertex shader to generate vertices)
    // This is a temporary solution until we implement proper vertex buffer support
    // We're using the hardcoded cube in the vertex shader, not the mesh data yet
    std::cout << "RenderMesh: Drawing hardcoded cube with 36 vertices (12 triangles)" << std::endl;
    try {
        vkCmdDraw(commandBuffer, 36, 1, 0, 0);
        std::cout << "RenderMesh: Cube draw command completed successfully" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "RenderMesh: Exception during vkCmdDraw: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "RenderMesh: Unknown exception during vkCmdDraw" << std::endl;
    }
    
    // Log that we're rendering with hardcoded vertices instead of mesh data
    std::cout << "RenderMesh: Currently using hardcoded cube vertices (mesh data ignored)" << std::endl;
    std::cout << "RenderMesh: Mesh has " << mesh.GetVertexCount() << " vertices and " 
              << mesh.GetIndexCount() << " indices" << std::endl;
    
    // Note: mesh data is currently ignored as we're using hardcoded vertices in the shader
    // Future implementation will use the mesh data properly
    if (texture) {
        std::cout << "RenderMesh: Texture provided but ignored in current implementation" << std::endl;
    } else {
        std::cout << "RenderMesh: No texture provided" << std::endl;
    }
...
VkShaderModule VulkanRenderer::CreateShaderModule(const std::vector<char>& code) {
    ...
    if (vkCreateShaderModule(static_cast<VkDevice>(m_device), &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
