...
bool VulkanBuffer::UpdateData(const void *data, size_t size, size_t offset) {
  if (!data) {
    std::cerr << "Error: VulkanBuffer::UpdateData - data pointer is null!"
              << std::endl;
    return false;
  }
  ...
  if (m_size == 0) {
    std::cerr << "Error: VulkanBuffer::UpdateData - buffer size is zero!"
              << std::endl;
    return false;
  }
  ...
  if (m_device != VK_NULL_HANDLE && m_memory != VK_NULL_HANDLE) {
    ...
    if (result == VK_SUCCESS) {
      ...
      std::cout << "Updated Vulkan buffer data: " << size << " bytes at offset "
                << offset << std::endl;
      return true;
    } else {
      std::cerr << "Failed to map Vulkan buffer memory for update" << std::endl;
      return false;
    }
  }
#endif
...
void *VulkanBuffer::Map() {
  ...
  if (m_device != VK_NULL_HANDLE && m_memory != VK_NULL_HANDLE) {
    VkResult result =
        vkMapMemory(m_device, m_memory, 0, m_size, 0, &m_mappedData);
    if (result == VK_SUCCESS) {
      return m_mappedData;
    }
  }
...
#ifdef AQUA_HAS_VULKAN
bool VulkanBuffer::CreateVulkanBuffer(VkDevice device,
                                      VkPhysicalDevice physicalDevice) {
  m_device = device;

  // Convert BufferType to VkBufferUsageFlags
  ...
  std::cout << "Created Vulkan buffer: " << m_size << " bytes" << std::endl;
  return true;
}
...
bool VertexBuffer::Create(const void *vertices, size_t size, uint32_t usage) {
  if (!vertices) {
    std::cerr << "Error: VertexBuffer::Create - vertices pointer is null!"
              << std::endl;
    return false;
  }
  ...
  if (size >
      100000000) { // 100MB should be more than enough for most vertex buffers
    std::cerr << "Error: VertexBuffer::Create - size seems unreasonably large: "
              << size << " bytes" << std::endl;
    return false;
  }
  ...
  m_buffer =
      BufferManager::Instance().CreateBuffer(size, BufferType::Vertex, usage);
  if (!m_buffer) {
    std::cerr
        << "Error: VertexBuffer::Create - failed to create underlying buffer!"
        << std::endl;
    return false;
  }

  if (!m_buffer->UpdateData(vertices, size)) {
    std::cerr << "Error: VertexBuffer::Create - failed to update buffer data!"
              << std::endl;
    m_buffer.reset();
    return false;
  }
  ...
  m_vertexCount =
      static_cast<uint32_t>(size / 12); // Assume 12 bytes per vertex

  std::cout << "Created vertex buffer: " << m_vertexCount << " vertices"
            << std::endl;
  return true;
}
...
bool VertexBuffer::UpdateData(const void *vertices, size_t size,
                              size_t offset) {
  if (!m_buffer)
    return false;
...
void VertexBuffer::Bind() { std::cout << "Binding vertex buffer" << std::endl; }
...
bool IndexBuffer::Create(const uint32_t *indices, uint32_t count,
                         uint32_t usage) {
  if (!indices) {
    std::cerr << "Error: IndexBuffer::Create - indices pointer is null!"
              << std::endl;
    return false;
  }

  if (count == 0) {
    std::cerr << "Error: IndexBuffer::Create - index count is zero!"
              << std::endl;
    return false;
  }
  ...
  if (count >
      1000000) { // 1 million indices should be more than enough for most cases
    std::cerr
        << "Error: IndexBuffer::Create - index count seems unreasonably large: "
        << count << std::endl;
    return false;
  }
  ...
  std::cout << "Creating index buffer with " << count << " indices (" << size
            << " bytes)" << std::endl;

  m_buffer =
      BufferManager::Instance().CreateBuffer(size, BufferType::Index, usage);
  if (!m_buffer) {
    std::cerr
        << "Error: IndexBuffer::Create - failed to create underlying buffer!"
        << std::endl;
    return false;
  }

  if (!m_buffer->UpdateData(indices, size)) {
    std::cerr << "Error: IndexBuffer::Create - failed to update buffer data!"
              << std::endl;
    m_buffer.reset();
    return false;
  }
  ...
  std::cout << "Created index buffer: " << m_indexCount << " indices"
            << std::endl;
  return true;
}
...
bool IndexBuffer::UpdateData(const uint32_t *indices, uint32_t count,
                             size_t offset) {
  if (!m_buffer)
    return false;
  size_t size = count * sizeof(uint32_t);
...
void IndexBuffer::Bind() { std::cout << "Binding index buffer" << std::endl; }
...
bool UniformBuffer::Create(size_t size, uint32_t usage) {
  ...
  m_buffer =
      BufferManager::Instance().CreateBuffer(size, BufferType::Uniform, usage);
  if (!m_buffer)
    return false;
...
void UniformBuffer::Destroy() { m_buffer.reset(); }
...
BufferManager::CreateVertexBuffer(const void *vertices, size_t size,
                                  uint32_t usage) {
...
BufferManager::CreateIndexBuffer(const uint32_t *indices, uint32_t count,
                                 uint32_t usage) {
...
BufferManager::CreateUniformBuffer(size_t size, uint32_t usage) {
...
BufferManager::CreateBuffer(size_t size, BufferType type, uint32_t usage) {
...
#ifdef AQUA_HAS_VULKAN
void BufferManager::SetVulkanDevice(VkDevice device,
                                    VkPhysicalDevice physicalDevice) {
  m_device = device;
