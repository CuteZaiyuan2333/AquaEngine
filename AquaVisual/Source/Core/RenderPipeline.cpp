...
bool RenderPipeline::Create(const PipelineCreateInfo& createInfo) {
    ...
    if (!createInfo.shaderProgram) {
        std::cerr << "Pipeline creation failed: No shader program provided" << std::endl;
        return false;
    }
...
#ifdef AQUA_HAS_VULKAN
bool RenderPipeline::CreateVulkanPipeline(VkDevice device, VkRenderPass renderPass) {
    m_device = device;
    ...
    rasterizer.rasterizerDiscardEnable = m_createInfo.rasterization.rasterizerDiscardEnable;
    rasterizer.polygonMode = static_cast<VkPolygonMode>(m_createInfo.rasterization.polygonMode);
    rasterizer.lineWidth = m_createInfo.rasterization.lineWidth;
    rasterizer.cullMode = static_cast<VkCullModeFlags>(m_createInfo.rasterization.cullMode);
    rasterizer.frontFace = static_cast<VkFrontFace>(m_createInfo.rasterization.frontFace);
    rasterizer.depthBiasEnable = m_createInfo.rasterization.depthBiasEnable;
    
    // 多重采样状态
    VkPipelineMultisampleStateCreateInfo multisampling{};
    multisampling.sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
    multisampling.sampleShadingEnable = m_createInfo.multisample.sampleShadingEnable;
    multisampling.rasterizationSamples = static_cast<VkSampleCountFlagBits>(m_createInfo.multisample.rasterizationSamples);
    ...
    depthStencil.depthCompareOp = static_cast<VkCompareOp>(m_createInfo.depthStencil.depthCompareOp);
    depthStencil.depthBoundsTestEnable = m_createInfo.depthStencil.depthBoundsTestEnable;
    ...
    colorBlending.logicOp = static_cast<VkLogicOp>(m_createInfo.colorBlend.logicOp);
...
std::shared_ptr<RenderPipeline> PipelineManager::CreatePipeline(const PipelineCreateInfo& createInfo) {
...
std::shared_ptr<RenderPipeline> PipelineManager::GetPipeline(const std::string& name) {
...
PipelineCreateInfo PipelineManager::CreatePBRPipelineInfo() {
    ...
    info.colorBlend.attachments[0].srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;
    info.colorBlend.attachments[0].dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
    info.colorBlend.attachments[0].colorBlendOp = VK_BLEND_OP_ADD;
    info.colorBlend.attachments[0].srcAlphaBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;
    info.colorBlend.attachments[0].dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
