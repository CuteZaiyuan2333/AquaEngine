...
void Renderer::UpdateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.proj = Math::Matrix4::Perspective(
    ...
    void* data;
    vkMapMemory(m_vulkanContext->GetDevice(), m_uniformBuffersMemory[currentImage], 0, sizeof(ubo), 0, &data);
    memcpy(data, &ubo, sizeof(ubo));
    vkUnmapMemory(m_vulkanContext->GetDevice(), m_uniformBuffersMemory[currentImage]);
}
...
bool Renderer::CreateGraphicsPipeline() {
    ...
    if (shaderStages.empty()) {
        std::cerr << "No shader stages available. Cannot create graphics pipeline." << std::endl;
        return false;
    }
    
    std::cout << "Shader modules created successfully, creating graphics pipeline..." << std::endl;
