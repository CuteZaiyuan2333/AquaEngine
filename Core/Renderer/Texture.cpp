...
// Use stb_image library to load images
#define STB_IMAGE_IMPLEMENTATION
#include "../../ThirdParty/stb/stb_image.h"

namespace AquaEngine {

// Static VulkanContext pointer for texture operations
static Aqua::Renderer::VulkanContext* s_vulkanContext = nullptr;

// Static command pool for texture operations
static VkCommandPool s_commandPool = VK_NULL_HANDLE;

void Texture::SetVulkanContext(Aqua::Renderer::VulkanContext* context) {
...
bool Texture::LoadFromFile(const std::string &filepath) {
  ...
  m_channels = 4; // Force RGBA usage

  std::cout << "Texture loaded successfully: " << m_width << "x" << m_height
            << " channels: " << channels << std::endl;

  // Create Vulkan texture
  bool success = CreateVulkanTexture(data, m_width, m_height);
...
bool Texture::CreateVulkanTexture(unsigned char* data, uint32_t width, uint32_t height) {
  ...
  // Create image view
  if (!CreateImageView(m_image, VK_FORMAT_R8G8B8A8_SRGB, m_imageView)) {
    std::cerr << "Failed to create image view!" << std::endl;
    return false;
  }
  
  // Create sampler
  if (!CreateSampler(m_sampler)) {
    std::cerr << "Failed to create sampler!" << std::endl;
    return false;
  }

  std::cout << "Vulkan texture created successfully: " << width << "x" << height << std::endl;
  return true;
}
...
VkCommandBuffer Texture::BeginSingleTimeCommands() {
  ...
  if (s_commandPool == VK_NULL_HANDLE) {
    ...
    poolInfo.queueFamilyIndex = s_vulkanContext->GetGraphicsQueueFamily();
    
    if (vkCreateCommandPool(s_vulkanContext->GetDevice(), &poolInfo, nullptr, &s_commandPool) != VK_SUCCESS) {
  ...
  vkAllocateCommandBuffers(s_vulkanContext->GetDevice(), &allocInfo, &commandBuffer);
...
void Texture::EndSingleTimeCommands(VkCommandBuffer commandBuffer) {
  ...
  vkQueueSubmit(s_vulkanContext->GetGraphicsQueue(), 1, &submitInfo, VK_NULL_HANDLE);
  vkQueueWaitIdle(s_vulkanContext->GetGraphicsQueue());

  // Use the global static command pool
  vkFreeCommandBuffers(s_vulkanContext->GetDevice(), s_commandPool, 1, &commandBuffer);
}

bool Texture::CreateImageView(VkImage image, VkFormat format, VkImageView& imageView) {
  ...
  if (vkCreateImageView(s_vulkanContext->GetDevice(), &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
    return false;
  }
...
bool Texture::CreateSampler(VkSampler& sampler) {
  ...
  if (vkCreateSampler(s_vulkanContext->GetDevice(), &samplerInfo, nullptr, &sampler) != VK_SUCCESS) {
    return false;
  }
